// Wraps send() to ALSO write to a file. Doesn't replace the original.
(function() {
  const findSym = (n) => {
  ...
  const fopenA = findSym('fopen'); if (!fopenA) return;
  const fwriteA = findSym('fwrite'); if (!fwriteA) return;
  const fflushA = findSym('fflush');
  ...
  // App's cache dir is universally writable to its own UID
  ...
  let fp;
  try { fp = fopenF(path, mode); }
  catch (e) { try { send({type:'log', msg:'[capture] fopen threw: '+e}); } catch(_) {}; return; }
  if (fp.isNull()) {
    try { send({type:'log', msg:'[capture] fopen returned NULL'}); } catch(_) {}
    return;
  }
  ...
  function writeLine(s) {
    try {
      ...
      fwriteF(buf, 1, s.length + 1, fp);
      if (fflushF) fflushF(fp);
    } catch (e) {}
  }

  // Confirm working
  writeLine('=== gadget script started: ' + new Date().toISOString() + ' ===');

  // Wrap the existing send rather than replacing it
  const _origSend = send;
  globalThis.send = function(payload, data) {
    try { writeLine('SEND ' + JSON.stringify(payload).substring(0, 8000)); } catch (e) {}
    try { return _origSend(payload, data); } catch (e) {}
  };
  writeLine('send() wrapped');
})();
